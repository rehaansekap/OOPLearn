...
<head>
    ...
    <title>Pertemuan 2 – Fase 3 – Penstrukturan Ide</title>
    ...
    <style>
        ...
        /* Mapping modal (pola sama dgn fase3.blade.php P1) */
        @keyframes fadeSlide {
...
<div class="flex min-h-screen" x-data="{ sidebarOpen: window.innerWidth >= 1024 }">
    ...
    <main class="flex-1 flex flex-col overflow-hidden">
        ...
        @include('_navbar', ['navTitle' => 'Pertemuan 2 – Fase 3 – Penstrukturan Ide'])
        ...
        <div class="bg-white rounded-[2.5rem] shadow-sm border border-gray-100 p-8 lg:p-10 min-h-[80vh] relative" data-aos="fade-up">
            ...
            <div class="flex items-center justify-between gap-3 mb-5 flex-wrap">
                ...
                <a href="{{ route('lesson') }}" target="_blank" onclick="tandaiMateriDibuka(2)"
                   class="inline-flex items-center gap-2 text-xs font-semibold text-blue-700 bg-blue-50 border border-blue-200
                          px-3 py-1.5 rounded-full hover:bg-blue-100 hover:border-blue-300 transition">
            ...
            <h2 class="text-2xl font-extrabold text-gray-900 mb-1">
                Penstrukturan Ide
                <span class="text-blue-600">— Karakter Gim (Hero &amp; Monster)</span>
            </h2>
            <p class="text-sm text-gray-500 mb-6">
                Susun blok kode untuk membuat class Hero yang mewarisi class Karakter dengan benar.
                Butuh referensi?
                <a href="{{ route('lesson') }}" target="_blank" onclick="tandaiMateriDibuka(2)" class="text-blue-600 font-medium hover:underline">Buka halaman Materi</a>.
            </p>
                ...
                <div class="flex items-center">
                    <div class="flex flex-col items-center gap-1 px-3">
                        <div class="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold border-2 transition-all"
                             style="{{ $n <= 3 ? 'background:#2563eb;border-color:#2563eb;color:white;' : 'background:white;border-color:#e5e7eb;color:#9ca3af;' }}">
                            {{ $n < 3 ? '✓' : $n }}
                        </div>
                        <span class="text-xs font-medium whitespace-nowrap"
                              style="{{ $n === 3 ? 'color:#1d4ed8;font-weight:700;' : 'color:#9ca3af;' }}">
                            {{ $nama }}
                        </span>
                    </div>
                    @if($n < 5)
                    <div class="h-0.5 w-8 flex-shrink-0" style="{{ $n < 3 ? 'background:#2563eb;' : 'background:#e5e7eb;' }}"></div>
                    @endif
                </div>
                @endforeach
            </div>
            ...
            <div class="bg-blue-50 border border-blue-100 rounded-2xl p-5 mb-6 flex gap-4">
                ...
                <div>
                    ...
                    <ul class="space-y-1 text-sm text-blue-900">
                        <li class="flex gap-2"><span class="text-blue-400 font-bold shrink-0">✓</span> Menyusun struktur class Hero yang mewarisi class Karakter dengan benar menggunakan block programming</li>
                        <li class="flex gap-2"><span class="text-blue-400 font-bold shrink-0">✓</span> Memahami penggunaan <code class="bg-blue-100 px-1 rounded text-xs">class Anak(Induk)</code> dan <code class="bg-blue-100 px-1 rounded text-xs">super().__init__()</code> di Python</li>
                    </ul>
                </div>
            </div>

            {{-- ═══════════════════════════════════════════════════════════
                 KONSEP CARDS (dipindahkan dari pertemuan-fase.blade.php,
                 blok @elseif($pertemuan==2 && $fase==3) baris 341-446 lama)
            ═══════════════════════════════════════════════════════════ --}}
            <div class="space-y-8 pb-10" data-aos="fade-up" data-aos-duration="550">
                ...
                <p class="text-gray-700 text-sm leading-relaxed">
                    Sekarang saatnya mempraktikkan konsep ini. Susun blok kode di bawah untuk membuat
                    <strong class="text-gray-900">class Hero</strong> yang mewarisi
                    <strong class="text-gray-900">class Karakter</strong> dengan benar!
                </p>
                ...
                <div id="workspacePanel"
                    ...
                    <div style="padding:10px 16px;background:#0a101e;border-bottom:1px solid rgba(255,255,255,0.05);display:flex;align-items:center;justify-content:space-between;flex-shrink:0;">
                        <div style="display:flex;align-items:center;gap:7px;">
                            ...
                            <span style="margin-left:8px;color:#475569;font-size:11px;font-weight:600;font-family:monospace;">inheritance.py</span>
                        </div>
                    ...
                    <div style="flex:1;padding:14px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:10px;">
                        ...
                        <div id="outSuccess" style="display:none;text-align:center;">
                            ...
                            <p style="color:#4ade80;font-size:12px;font-weight:700;line-height:1.5;">
                                Pewarisan berhasil!<br>
                                <span style="font-size:10px;font-weight:500;opacity:.75;">Hero mewarisi Karakter.</span>
                            </p>
                        </div>
            ...
            <div id="lanjutBox"
                 style="display:none;"
                 class="mt-5 flex items-center justify-between gap-4 bg-blue-50 border border-blue-200 rounded-2xl px-6 py-4">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <p class="text-sm font-bold text-blue-800">Penstrukturan Ide selesai!</p>
                        <p class="text-xs text-blue-600">Class Hero berhasil mewarisi Karakter. Lanjutkan ke Fase 4 untuk mempraktikkannya.</p>
                    </div>
                </div>
                ...
                <div class="flex items-center gap-3">
                    ...
                    <form method="POST" action="{{ route('p2.fase3.complete') }}">
                        @csrf
                        <button type="submit"
                                class="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 rounded-xl font-semibold text-sm transition shadow-sm whitespace-nowrap">
...
><span class="code-line" data-map-id="struktur-class" onmouseenter="highlightPair('struktur-class')" onmouseleave="clearHighlight()">class Karakter:</span>
<span class="code-line" data-map-id="struktur-class" onmouseenter="highlightPair('struktur-class')" onmouseleave="clearHighlight()">    def __init__(self, nama, nyawa):</span>
...
<span class="code-line" data-map-id="struktur-class" onmouseenter="highlightPair('struktur-class')" onmouseleave="clearHighlight()">        self.nyawa = nyawa</span>
<span class="code-line" data-map-id="method" onmouseenter="highlightPair('method')" onmouseleave="clearHighlight()">    def info(self):</span>
<span class="code-line" data-map-id="method" onmouseenter="highlightPair('method')" onmouseleave="clearHighlight()">        return f"{self.nama} punya {self.nyawa} nyawa"</span>
<span class="code-line" data-map-id="pewarisan" onmouseenter="highlightPair('pewarisan')" onmouseleave="clearHighlight()">class Hero(Karakter):</span>
<span class="code-line" data-map-id="pewarisan" onmouseenter="highlightPair('pewarisan')" onmouseleave="clearHighlight()">    def __init__(self, nama, nyawa, energi_sihir):</span>
<span class="code-line" data-map-id="pewarisan" onmouseenter="highlightPair('pewarisan')" onmouseleave="clearHighlight()">        super().__init__(nama, nyawa)</span>
<span class="code-line" data-map-id="struktur-class" onmouseenter="highlightPair('struktur-class')" onmouseleave="clearHighlight()">        self.energi_sihir = energi_sihir</span>
<span class="code-line" data-map-id="method" onmouseenter="highlightPair('method')" onmouseleave="clearHighlight()">    def info(self):</span>
<span class="code-line" data-map-id="method" onmouseenter="highlightPair('method')" onmouseleave="clearHighlight()">        return f"{self.nama} (Hero) - Mana: {self.energi_sihir}"</span></code></pre>
                        </div>
...
// ═════════════════════════════════════════════════════════════════════════════
// BLOCK DATA — domain Karakter / Hero / Monster (Inheritance)
// ═════════════════════════════════════════════════════════════════════════════

const BLOCKS = {
    struktur: [
        { id:'class', label:'class Karakter:',                 icon:'📦', color:'#4C97FF', tag:'Class' },
        { id:'init',  label:'def __init__(self, nama, nyawa):', icon:'⚙️', color:'#3d82e0', tag:'Init'  },
        { id:'pass',  label:'pass',                              icon:'–',  color:'#475569', tag:'Pass'  },
    ],
    pewarisan: [
        { id:'extends', label:'class Hero(Karakter):',            icon:'🧬', color:'#db2777', tag:'ChildClass' },
        { id:'super',   label:'super().__init__(nama, nyawa)',    icon:'🔗', color:'#be185d', tag:'SuperInit'  },
    ],
    method: [
        { id:'override', label:'def info(self):',    icon:'💬', color:'#ea580c', tag:'Method' },
        { id:'return',   label:'return f"..."',       icon:'↩️', color:'#dc4e08', tag:'Return' },
    ],
    akses: [
        { id:'attr', label:'self.energi_sihir = nilai', icon:'✏️', color:'#9333ea', tag:'Atribut' },
    ],
};

const CAT_COLORS = {
    struktur:'#4C97FF', pewarisan:'#db2777', method:'#ea580c', akses:'#9333ea',
};
...
// ═════════════════════════════════════════════════════════════════════════════
// RUN — validasi Inheritance (deteksi kelengkapan tag, bukan urutan/kebenaran)
// ═════════════════════════════════════════════════════════════════════════════
...
function runProgram() {
    ...
    const hasMethod     = tags.includes('Method');

    document.getElementById('outEmpty').style.display = 'none';

    if (hasClass && hasChildClass && hasSuperInit && hasMethod) {
        ...
        const missing = [];
        if (!hasClass)      missing.push('class induk (Karakter)');
        if (!hasChildClass) missing.push('class anak yang mewarisi (Hero)');
        if (!hasSuperInit)  missing.push('pemanggilan super().__init__()');
        if (!hasMethod)     missing.push('method di class anak');
